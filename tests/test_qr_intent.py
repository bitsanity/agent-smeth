...
class TestQrIntent(unittest.TestCase):
    ...
    @patch("actions.main._render_qr_terminal_utf8", return_value="<qr utf8>")
    def test_show_qr_that_says_message(self, _mock_render, _mock_adilos, _mock_qr, _mock_zbar):
        ...
        self.assertEqual(result["response"], "Rendered QR code for terminal display.")
        self.assertEqual(result["data"]["qr"]["message"], "Hello World")
        self.assertEqual(result["data"]["qr"]["utf8"], "<qr utf8>")
        self.assertEqual(result["data"]["qr"]["format"], "UTF8")
