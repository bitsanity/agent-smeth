"""                                                                          
   ...
   - Full ENS resolution and ERC20 calldata encoding require additional         
 config/deps.                                                                   
   """                                                                          
 ...
 "                                                                              
   ...
   "Please remove it and retry with only public information (tx hash, address,  
 calldata, ABI)."                                                               
   )                                                                            
   return None                                                                  
   ...
   def _jsonrpc(rpc_url: str, method: str, params: list[Any], timeout_s: int =  
 20) -> Any:                                                                    
   if rpc_url.startswith("ws://") or rpc_url.startswith("wss://"):              
   raise RuntimeError("This action only supports HTTP(S) JSON-RPC endpoints     
 (not ws://).")                                                                 
   ...
   def _etherscan_get(api_url: str, api_key: str, params: Dict[str, str],       
 timeout_s: int = 20) -> Dict[str, Any]:                                        
   ...
   return isinstance(name, str) and "." in name and                             
 name.lower().endswith(".eth")                                                  
   ...
   # fallback: unknown                                                          
   return {"raw": amount, "note": "Could not parse amount into wei. Provide     
 like '0.01 ETH' or '1337 finney'."}                                            
   ...
   etherscan_api_url = etherscan_api_url or os.getenv("ETHERSCAN_API_URL") or   
 "https://etherscan.io"                                                         
   ...
   "ethereum" in intent_l or re.search(r"\beth\b", intent_l) or "agent-smeth"   
 in intent_l                                                                    
   ):                                                                           
   ...
   data_out["sources"].append("ethe rscan:stats.ethprice")                      
   data_out["ethprice_raw"] = resp                                              
   # best-effort extraction                                                     
   result = resp.get("result", {}) if isinstance(resp, dict) else {}            
   price_usd = result.get("ethusd") if isinstance(result, dict) else None       
   out_obj = {"price_usd": price_usd, "note": "Gas price is network-dependent;  
 use RPC for live gas price."}                                                  
   return {                                                                     
   ...
   return {                                                                     
   "response": (                                                                
   "I couldn’t fetch ETH price because Etherscan is not configured (missing     
 ETHERSCAN_API_KEY) "                                                           
   "or the request failed. Provide etherscan_api_key (or set env                
 ETHERSCAN_API_KEY), "                                                          
   ...
   # ---------------------------                                                
   if ("explain" in intent_l or "receipt" in intent_l or "transaction" in       
 intent_l) and tx_hash:                                                         
   try:                                                                         
   tx = _jsonrpc(rpc_url, "eth_getTransactionByHash", [tx_hash])                
   rcpt = _jsonrpc(rpc_url, "eth_getTransactionReceipt", [tx_hash])             
   data_out["sources"].append("rpc: eth_getTransactionByHash,eth_get            
 TransactionReceipt")                                                           
   data_out["tx"] = tx                                                          
   ...
   "value_wei": _hex_to_int((tx or {}).get("value")) if isinstance(tx, dict)    
 else None,                                                                     
   "status": (rcpt or {}).get("status") if isinstance(rcpt, dict) else None,    
   "gas_used": _hex_to_int((rcpt or {}).get("gasUsed")) if isinstance(rcpt,     
 dict) else None,                                                               
   "logs_count": len((rcpt or {}).get("logs", [])) if isinstance(rcpt, dict)    
 else None,                                                                     
   }                                                                            
   return {"response": "Fetched transaction + receipt via JSON-RPC.", "data":   
 {**data_out, "summary": summary}}                                              
   except Exception as e:                                                       
   return {                                                                     
   "response": f"RPC tx lookup failed. If your default is ws://127.0.0.1:8546,  
 provide an HTTP(S) rpc_url. Error: {e}",                                       
   "data": data_out,                                                            
   }                                                                            
   ...
   from_ and _looks_like_ens(from_) or to and _looks_like_ens(to) or ".eth" in  
 intent_l                                                                       
   ):                                                                           
   return {                                                                     
   "response": (                                                                
   "ENS resolution requires calling the ENS registry/resolver contracts via RPC 
 (eth_call) "                                                                   
   "and knowing the ENS registry address for the target chain. "                
   "This action is currently configured as a placeholder; provide/implement     
 ENS_REGISTRY_ADDRESS "                                                         
   "and resolver logic (or add a dependency like web3.py) to fully resolve .eth 
 names."                                                                        
   ),                                                                           
   "data": {**data_out, "ens": {"note": "placeholder", "requested_from": from_, 
 "requested_to": to}},                                                          
   }                                                                            
   ...
   # ---------------------------                                                
   if ("build" in intent_l or "construct" in intent_l or "create" in intent_l)  
 and ("send" in intent_l or "transfer" in intent_l) and to and amount and       
 (token is None):                                                               
   ...
   data_out["sources"].append("rpc: eth_getTransactionCount,eth_gasP rice")     
   ...
   # ---------------------------                                                
   if token and ("erc20" in intent_l or "token" in intent_l or "transfer" in    
 intent_l):                                                                     
   return {                                                                     
   "response": (                                                                
   "ERC-20 transfer construction requires the token contract address on this    
 chain and "                                                                    
   "calldata encoding for transfer(address,uint256). "                          
   "This action currently provides a placeholder. Add a token-address registry  
 and an ABI encoder "                                                           
   "(e.g., eth-abi/eth-utils) to output the full unsignedtx."                   
   ),                                                                           
   "data": {**data_out, "erc20": {"token": token, "from": from_, "to": to,      
 "amount": amount, "note": "placeholder"}},                                     
   }                                                                            
   ...
   "High-level steps: generate an ADILOS challenge; display as QR; user         
 signs/responds via SIMPLETH; "                                                 
   "scan response QR; derive verified pubkey/address; optionally reverse-lookup 
 ENS."                                                                          
   ),                                                                           
   "data": {**data_out, "verify_flow": {"challenge": "platform-dependent",      
 "note": "hook"}},                                                              
   }                                                                            
   ...
   return {                                                                     
   "response": (                                                                
   "I can help with ETH price (Etherscan), tx explanation (RPC), and building   
 unsigned tx objects. "                                                         
   "For full ENS resolution, ERC-20 calldata building, QR signing, and          
 broadcast/signing, additional "                                                
   "runtime integrations are needed (ENS contract addresses, ABI encoder,       
 camera/QR IO, and signer)."                                                    
   ),                                                                           
   "data": data_out,                                                            
   }
